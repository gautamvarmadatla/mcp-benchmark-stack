...
| S8 | **Synthetic injection**: denial occurs, no server-side trace produced | observability | observability | violating |
...
- **S8 is a benchmarked synthetic observability-gap injection**, not an organically surfaced runtime signal. The client explicitly injects the `OBSERVABILITY_GAP` string when the server produces no trace. This is intentional and labeled as such — do not describe it as a naturally detected observability failure.
